...
function createOrbit(size) {
  ...
  $("canvas").drawEllipse({
    ...
    dragstop: function(layer) {
      ...
      var num = parseInt(orbit.name.replace(/[^0-9^\.]/g,""));
      pointList[num*2] = orbit.x;
      pointList[num*2+1] = orbit.y;
      console.log(pointList);
    }
  });
  pointList[orbitCount*2] = mouseX;
  pointList[orbitCount*2+1] = mouseY;
  console.log(pointList);
  $("canvas").drawEllipse({
    fillStyle: "white",
    x: mouseX,
...
$("#canvas").on("deleteEvent", function(e, grp, nam) {
  ...
  delete pointList[p*2];
  delete pointList[p*2+1];
